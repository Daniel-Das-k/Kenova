...
class SubjectDetector:
    def __init__(self):
        self.subject_prompts = {
            'mathematics': {
                'base': """From the provided document, extract mathematical topics and subtopics.
Focus on equations, theorems, and mathematical concepts.
Generate questions that test both computational and conceptual understanding.
Ensure questions span different difficulty levels (basic calculations to complex proofs).
Total marks should be 100.""",
                'short': "Generate numerical problems requiring step-by-step solutions. Include basic calculations, algebraic manipulations, and simple proofs.",
                'long': "Create comprehensive proof-based questions and complex problem-solving scenarios that require detailed mathematical reasoning.",
                'case': "Design applied mathematics scenarios that integrate multiple concepts and require systematic problem-solving approach."
            },
            'physics': {
                'base': """From the provided document, extract physics concepts, laws, and principles.
Focus on both theoretical understanding and numerical problem-solving.
Include questions about experimental design and data analysis.
Generate questions that test conceptual understanding and mathematical applications.
Total marks should be 100.""",
                'short': "Create numerical physics problems with proper units. Include conceptual questions about physical phenomena.",
                'long': "Design experimental analysis questions and theoretical derivations. Include detailed problem-solving scenarios.",
                'case': "Create real-world physics applications with experimental data analysis and practical implications."
            },
            'biology': {
                'base': """From the provided document, extract biological concepts, processes, and systems.
Focus on both structural and functional aspects of biological systems.
Include questions about experimental design and data interpretation.
Generate questions that test understanding of biological processes and their relationships.
Total marks should be 100.""",
                'short': "Generate diagram-based questions focusing on biological structures and functions. Include process identification questions.",
                'long': "Create comprehensive questions about biological processes and systems. Include experimental analysis and case studies.",
                'case': "Design scenarios involving biological experiments, data analysis, and real-world applications."
            },
            'history': {
                'base': """From the provided document, extract historical events, periods, and their significance.
Focus on cause-and-effect relationships and historical analysis.
Include questions about primary sources and historical interpretation.
Generate questions that test both factual knowledge and analytical skills.
Total marks should be 100.""",
                'short': "Create questions focusing on specific historical events, dates, and key figures. Include source-based questions.",
                'long': "Generate analytical questions about historical periods and their significance. Include comparison and evaluation questions.",
                'case': "Design historical case studies requiring analysis of multiple sources and interpretation of historical evidence."
            }
        }
    ...
    def detect_subject(self, file_path):
        ...
        # Initialize the LLM with Gemini Pro
        model = genai.GenerativeModel('gemini-pro')
        
        # Create a prompt for subject detection
        detection_prompt = f"""Analyze the following text and determine which subject it belongs to: mathematics, physics, biology, or history. 
        Consider the following characteristics for each subject:
        
        Mathematics:
        - Mathematical equations, formulas, and symbols
        - Abstract concepts and proofs
        - Numerical problems and calculations
        - Geometric or algebraic concepts
        
        Physics:
        - Physical laws and principles
        - Scientific experiments and measurements
        - Forces, energy, and motion concepts
        ...
        Return only the subject name in lowercase, nothing else."""
        
        # Get the subject prediction
        response = model.generate_content(detection_prompt)
        predicted_subject = response.text.strip().lower()
        
        # Validate the prediction
        if predicted_subject not in self.subject_prompts:
            # Default to mathematics if prediction is uncertain
            return 'mathematics'
        
        return predicted_subject

    def get_subject_prompts(self, subject):
        return self.subject_prompts.get(subject, self.subject_prompts['mathematics'])
...
class QuestionPaperGenerator:
    def __init__(self, file):
        ...
        # Detect subject and get appropriate prompts
        self.subject = self.subject_detector.detect_subject(file)
        self.prompts = self.subject_detector.get_subject_prompts(self.subject)
        self.uuid = str(uuid.uuid4())
        self.CHROMA_DB_PATH = f"./chroma_db_{self.uuid}"
        self.CHROMA_COLLECTION = f"autogen_docs_{self.uuid}"
    ...
    def create_agents(self):
        ...
        self.short_answer = AssistantAgent(
            ...
            system_message="""Generate exactly 10 concise questions worth 2 marks each, focusing on fundamental concepts and basic understanding.
            Questions should be clear, direct, and test essential knowledge.
            The answer should be precise, between 35-50 words.
        ...
        self.long_answer =AssistantAgent(
            name="10_marks",
            is_termination_msg=self.termination_msg,
            system_message="""Generate exactly 5 comprehensive questions worth 13 marks each.
            Each question should require in-depth analysis, critical thinking, or detailed problem-solving.
            Mix different types of questions (theoretical, analytical, numerical when applicable).
            The answer should be thorough, between 900-1000 words.
            Format each question as:
            {
                "question_number": <number>,
                "question_text": "<question>",
        ...
        self.case_study = AssistantAgent(
            ...
            The answer should be comprehensive and well-structured.
            For a 15-mark question, provide a detailed answer of approximately 1000 words.
            Scale answer length proportionally for questions with different mark allocations.
            Ensure answers demonstrate complete mastery of the subject matter.
            
            {
                "section_title": "Case Study",
                "total_marks": 15,
                "background": "<detailed scenario description>",
                "problem_statement": "<specific problem or challenge>",
                "supporting_data": [
                ...
                "questions": [
                    {
                        "question_number": 1,
                        "question_text": "<analytical question>",
                        "answer": "<comprehensive answer>",
                        "marks": <mark allocation>
                    }
                ]
            }
            The case study should present a real-world scenario that tests critical thinking and application of concepts.""",
            llm_config=self.llm_config,
        )
